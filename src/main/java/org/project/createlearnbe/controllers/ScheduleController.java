...
@Tag(
    ...
    description =
        "APIs for managing class schedules, including creation, updates, retrieval, and deletion.")
@RestController
...
public class ScheduleController {
  ...
  @Operation(
      ...
      description =
          "Adds a new class schedule (e.g., 'Tuesday - 8am to 9am') linked to a specific class.")
  @ApiResponses({
    @ApiResponse(
        responseCode = "200",
  ...
  @PostMapping
  public ResponseEntity<ApiWrapper<ScheduleResponse>> create(@RequestBody ScheduleRequest request) {
  ...
  @Operation(
      summary = "Get schedule by Class ID",
      description = "Retrieves the details of a single schedule by its class.")
  @ApiResponses({
    @ApiResponse(
        responseCode = "200",
  ...
  @GetMapping("/{id}")
  public ResponseEntity<ApiWrapper<ApiPage<ScheduleResponse>>> getByClassId(
      @PathVariable Long id, Pageable pageable) {
    return ResponseEntity.ok(ApiWrapper.success(scheduleService.getByClassId(id, pageable)));
  }
  ...
  @ApiResponse(
      ...
      content = @Content(schema = @Schema(implementation = ScheduleResponse.class)))
  @GetMapping
  public ResponseEntity<ApiWrapper<ApiPage<ScheduleResponse>>> getAll(Pageable pageable) {
    return ResponseEntity.ok(ApiWrapper.success(scheduleService.getAll(pageable)));
  }
}
