...
public class AuthController {
  ...
  @Operation(
      summary = "User login",
      description =
          "Authenticate a user with username and password, returning access and refresh tokens.")
  @ApiResponses({
    @ApiResponse(
        responseCode = "200",
  ...
  public ResponseEntity<ApiWrapper<Map<String, String>>> refresh(
      @RequestBody(
              description = "Refresh token in JSON format (key: refreshToken)",
              required = true)
          @org.springframework.web.bind.annotation.RequestBody
          Map<String, String> request) {
    return ResponseEntity.ok(
        ApiWrapper.success(authenticationService.refresh(request.get("refreshToken"))));
  }
}
